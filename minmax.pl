...
g(Jogo):- 
	[Jogo], 
	estado_inicial(Ei), 
	minimax_decide(Ei,Op),
	write(Op),nl.

% Decide qual é a melhor jogada num estado do jogo
% minimax_decide(Estado, MelhorJogada)
% se é estado terminal não joga 

minimax_decide(Ei,terminou):- 
	terminal(Ei).


%Para cada estado sucessor de Ei calcula o valor minimax do estado em que está
%Opf é o operador com maior valor

minimax_decide(Ei,Opf):- 
...
% Se o estado é terminal o valor é dado pela função de utilidade
